...
Omni SuperDev packaging note: this bundled copy adapts migrator path examples so they resolve from `omni-superdev/references/bundled/migrate-to-codex/` instead of requiring a separate `migrate-to-codex/` skill installation.
...
Choose the migrator command.
   ...
   ```bash
   OMNI_SUPERDEV_SKILL='<path-to-installed-omni-superdev>'
   MIGRATE_TO_CODEX="python3 $OMNI_SUPERDEV_SKILL/references/bundled/migrate-to-codex/scripts/migrate-to-codex.py"
   ```
